...
export const makeVanillaTheme = (braidTokens: BraidTokens) => {
  ...
  const getInlineFieldSize = (size: keyof typeof textSize) => {
    const scale = parseInt(textSize[size].mobile.lineHeight, 10) / 52;
  ...
  const resolvedTokens = {
    ...
    inlineFieldSize: {
      standard: getInlineFieldSize('large'),
      small: getInlineFieldSize('standard'),
    },
